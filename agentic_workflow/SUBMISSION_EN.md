...
The selected-product comparison now uses the team's `description_module` through
an adapter in `agentic_workflow`. After `Compare #1 and #2`, the page shows listed
facts, expandable source quotes, and separate advice for the shopper's preferences.
Inferences and unknown values are labeled. Without a model provider, the module
returns a direct-facts preview; configure `--model-provider local` or `deepseek`
for extraction, objective comparison and personalized explanations. See
